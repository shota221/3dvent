...
namespace App\Services\Support\Math;

use Tests\Unit\Complex;
...
class Fft
{
     ...
     * @return array<double> get the absolute value of FFT
     */
    public function getAbsFFT($w)
    {
        for ($i = 0; $i < $this->dim; $i++)
            $temp[$i] = Complex::Cabs($w[$i])/$this->dim*2;
            //データの点数によってFFTの結果で得られる振幅が変わってしまうためデータ点数で割ることで正規化。
        return $temp;
    }
